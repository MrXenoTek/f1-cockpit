...
const DICT = {
  fr: { weather: "🌤️ Météo", radio: "📻 Radio", ctrl: "🏁 Dir.", pits: "🛠️ Stands", timing: "⏱️ Temps", speed: "Vitesse", throttle: "Accélérateur", brake: "Frein", gear: "Rapport", ers: "ERS", live: "DIRECT", ranking: "Classement", lap: "Tour", dom: "DOMINATION SPATIALE", ers_state: "ÉTAT ERS", faster: "Plus", deploy: "Déploiement", harvest: "Récolte", clip: "Clipping", superclip: "Super Clip", coast: "Lift&Coast", tel: "TÉLÉMÉTRIE" },
  en: { weather: "🌤️ Weather", radio: "📻 Radio", ctrl: "🏁 Ctrl", pits: "🛠️ Pits", timing: "⏱️ Timing", speed: "Speed", throttle: "Throttle", brake: "Brake", gear: "Gear", ers: "ERS", live: "LIVE", ranking: "Standings", lap: "Lap", dom: "SPATIAL DOMINATION", ers_state: "ERS STATE", faster: "More", deploy: "Deploy", harvest: "Harvest", clip: "Clipping", superclip: "Super Clip", coast: "Lift & Coast", tel: "TELEMETRY" }
};
...
export default function App(){
  ...
  const loadSessionAggregateData = useCallback(async (isInitial = false) => {
    ...
    try {
        ...
        const [drv, ld, pos, st, pt, iv, rad, rc, wx] = await Promise.all([
            fetchApi("drivers",{session_key:sk},isInitial?log:null), fetchApi("laps",{session_key:sk},isInitial?log:null),
            fetchApi("position",{session_key:sk},isInitial?log:null), fetchApi("stints",{session_key:sk},isInitial?log:null),
            fetchApi("pit",{session_key:sk},isInitial?log:null), fetchApi("intervals",{session_key:sk},isInitial?log:null),
            fetchApi("team_radio",{session_key:sk},isInitial?log:null), fetchApi("race_control",{session_key:sk},isInitial?log:null),
            fetchApi("weather",{session_key:sk},isInitial?log:null)
        ]);

        if(Array.isArray(drv)){ const u=[...new Map(drv.map(x=>[x.driver_number,x])).values()]; setDrivers(u); if(isInitial && u.length) setSelDrv(u[0].driver_number); }
        if(Array.isArray(ld)){ setLaps(ld); const newMax = Math.max(...ld.map(l=>l.lap_number||0), 1); setMaxLap(newMax); if (!isInitial && isLive && curLapRef.current === maxLapRef.current && newMax > maxLapRef.current) setCurLap(newMax); }
        if(Array.isArray(pos)&&pos.length){ const sorted=[...pos].sort((a,b)=>new Date(a.date)-new Date(b.date)); const state={};const snaps=[]; for(const p of sorted){state[p.driver_number]=p.position;snaps.push({date:p.date,state:{...state}});} setPosSnaps(snaps); }
        if(Array.isArray(st)) setStints(st); if(Array.isArray(pt)) setPits(pt); if(Array.isArray(iv)) setIntervals(iv);
        if(Array.isArray(rad)) setRadios(rad.sort((a,b)=>new Date(a.date)-new Date(b.date))); if(Array.isArray(rc)) setRCtrl(rc);
  ...
  useEffect(()=>{
    ...
    const fetchDriverData = async (drv, setter) => {
        ...
        if(Array.isArray(d)) {
            ...
            for(const pt of sorted) { const tm = new Date(pt.date).getTime(); if(tm - lastTime > 100) { unique.push(pt); lastTime = tm; } }
  ...
  const telChart=useMemo(()=>{
    ...
    for(let i=0; i<200; i++) {
        ...
        if(!p1) continue;
        const pt = { 
            i, speed1: p1.speed, throttle1: p1.throttle, brake1: p1.brake, gear1: p1.n_gear, ers1: getErsVal(p1, d1[i1-1])
        };
        if(d2.length) {
            const i2 = Math.floor((i/199) * (d2.length - 1));
            const p2 = d2[i2];
            if(p2) { 
              pt.speed2 = p2.speed; pt.throttle2 = p2.throttle; pt.brake2 = p2.brake; pt.gear2 = p2.n_gear; pt.ers2 = getErsVal(p2, d2[i2-1]); 
              pt.delta = pt.speed1 - pt.speed2; // <--- NOUVELLE LIGNE ICI
            }
        }
        res.push(pt);
    }
  ...
  // EMPÊCHE LA DOUBLE SÉLECTION VS
  const handleDriverSelect = (dn) => {
    setSelDrv(dn);
    if (cmpDrv === dn) setCmpDrv(null); 
  };
  ...
  const gridStyle = isMobile ? { display: "flex", flexDirection: "column", height: "auto", minHeight: "100vh" } : { display: "grid", gridTemplateColumns: "220px 1fr 340px", gridTemplateRows: "44px 1fr", gridTemplateAreas: `"header header header" "sidebar main rightpanel"`, height: "100vh", overflow: "hidden" };

  return <div style={{ "--f":"'JetBrains Mono','SF Mono','Fira Code',monospace", background: "#0a0a0a", color: "#eee", fontFamily: "var(--f)", ...gridStyle }}>
    <header style={{gridArea: "header", padding:"8px 14px", display:"flex", alignItems:"center", gap:10, borderBottom:"1px solid #181818", background:"#0d0d0d", flexWrap: "wrap", zIndex: 10}}>
      ...
      <div style={{display:"flex",gap:6,flex:1,justifyContent:"flex-end",alignItems:"center", flexWrap:"wrap"}}>
        ...
        <button onClick={()=>setIsLive(!isLive)} style={{background:isLive?"#2a0a0a":"#1a1a1a", border:`1px solid ${isLive?"#E8002D":"#333"}`, color:isLive?"#E8002D":"#666", padding:"4px 8px", borderRadius:4, fontSize:10, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:4}}>
            {isLive && <div style={{width:6, height:6, borderRadius:"50%", background:"#E8002D", animation:"pulse 1.5s infinite"}}/>} {t("live")}
        </button>
        ...
        <select value={selMeet?.meeting_key||""} onChange={e=>{setSelMeet(meetings.find(m=>m.meeting_key===+e.target.value)||null);setSelSess(null);}} style={{...ss, minWidth:140, maxWidth:200}}><option value="">Grand Prix...</option>{meetings.map(m=><option key={m.meeting_key} value={m.meeting_key}>{m.meeting_name}</option>)}</select>
    ...
    {/* FIX : Utilisation stricte de CSS Grid avec minHeight:0 pour forcer le scroll de la télémétrie */}
    <div style={{gridArea: "main", display:"grid", gridTemplateRows: isMobile ? "auto 360px" : "1fr 400px", overflow: "hidden", minHeight: 0}}>
      <div style={{position:"relative", background:"#0e0e0e", minHeight: 0, overflow: "hidden"}}>
        <TrackMap trackX={trackX} trackY={trackY} corners={corners} ersSegs={ersSegs} mapMetric={mapMetric} setMapMetric={setMapMetric} currentLap={curLap} driverDots={driverDots} selDrv={selDrv} cmpDrv={cmpDrv} onSelect={handleDriverSelect} telChart={telChart} c1={c1} c2={c2} selDrvObj={selDrvObj} cmpDrvObj={cmpDrvObj} hoveredIndex={hoveredIndex} s1Ratio={s1Ratio} s2Ratio={s2Ratio} t={t} />
      ...
      <div style={{minHeight: 0, overflow: "hidden"}}>
        <Telemetry curLap={curLap} is26={is26} selDrvObj={selDrvObj} cmpDrvObj={cmpDrvObj} c1={c1} c2={c2} cmpDrv={cmpDrv} telStatus={telStatus} telChart={telChart} setHoveredIndex={setHoveredIndex} selLapData={selLapData} cmpLapData={cmpLapData} s1Ratio={s1Ratio} s2Ratio={s2Ratio} t={t} lapSC={lapSC} />
      </div>
    </div>

    {/* FIX : overflow:hidden et height:100% sur le wrapper pour forcer le scroll à l'intérieur de RightPanel */}
    <div style={{gridArea: "rightpanel", borderLeft: isMobile ? "none" : "1px solid #131313", height: isMobile ? "auto" : "100%", overflow: "hidden"}}>
      <RightPanel tab={tab} setTab={setTab} filtRadios={filtRadios} rCtrl={rCtrl} weather={weather} curWeather={curWeather} drivers={drivers} maxLap={maxLap} rfDrv={rfDrv} setRfDrv={setRfDrv} rfLap={rfLap} setRfLap={setRfLap} ss={ss} t={t} pits={pits} stints={stints} selDrv={selDrv} laps={laps} bestSectors={bestSectors} />
    </div>
  </div>;
}